...
length(grep("announcement_RRs", df$manual_tags))
```
...
```{r}
# find everything related to IPA

ipa <- lapply(df, function (x) grep("IPA", x, value = T))

ipa <- unlist(ipa)

names(ipa)[grep("pro_opin", names(ipa))] <- "opinion_pro"

names(ipa)[grep("empirical_con", names(ipa))] <- "empirical_con"

names(ipa)[grep("critique", names(ipa))] <- "critique_responses"

names(ipa)[grep("empirical_pro", names(ipa))] <- "empirical_pro"

ipa <- split(unname(ipa), names(ipa))

# convert list of unequal lengths to dataframe
...
# table cols with many observations separately
kable(ipa$critique_responses, col.names = "critique")

kable(ipa$opinion_pro, col.names = "opinion_pro")

# table cols with few observations
select(ipa, c(empirical_con, empirical_pro)) %>%
  kable()
