...
# tried to write a function but couldn't get it to work even when used separate grep & select column arguments

df[grep("IPA", df[, "empirical_pro"], ignore.case = T), ] %>%
  select(c(citekey, empirical_pro)) %>%
  .[!duplicated(.), ]%>%
  kable()

df[grep("IPA", df$empirical_con, ignore.case = T), ] %>%
  select(c(citekey, empirical_con)) %>%
  .[!duplicated(.), ]%>%
  kable()

df[grep("IPA", df$pro_opinion, ignore.case = T), ] %>%
  select(c(citekey, pro_opinion)) %>%
  .[!duplicated(.), ]%>%
  kable()

df[grep("IPA", df$critique_responses, ignore.case = T), ] %>%
  select(c(citekey, critique_responses)) %>%
  .[!duplicated(.), ]%>%
  kable()
